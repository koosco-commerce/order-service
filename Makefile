...
k8s-status: ## Show Kubernetes resources status
	...
	@kubectl get pods -n commerce -l app=order-service-mariadb
	@echo ""
	@echo "$(GREEN)Services:$(NC)"
	@kubectl get svc -n commerce -l service=order-service
	@echo ""
	@echo "$(GREEN)HPA:$(NC)"
	@kubectl get hpa -n commerce -l app=order-service
	@echo ""
	...
	@kubectl get deployments -n commerce -l app=order-service-mariadb
	@echo ""
	@echo "$(GREEN)PVC (Persistent Volume Claims):$(NC)"
	@kubectl get pvc -n commerce -l app=order-service-mariadb
...
k8s-scale: ## Scale application (usage: make k8s-scale REPLICAS=3)
	...
	@kubectl scale deployment/order-service -n commerce --replicas=$(REPLICAS)
