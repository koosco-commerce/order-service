...
help: ## Show this help message
	@echo "$(GREEN)Order Service - Available Commands$(NC)"
	@echo ""
...
clean: ## Clean up local containers and data
	...
	@docker rm -f order-service-test 2>/dev/null || true
...
docker-build: ## Build Docker image only
	...
	@echo "$(GREEN)✓ Image built: order-service:latest$(NC)"
...
k8s-status: ## Show Kubernetes resources status
	...
	@kubectl get svc -n commerce -l app=order-service
	@echo ""
	@echo "$(GREEN)HPA:$(NC)"
	@kubectl get hpa -n commerce
	@echo ""
	@echo "$(GREEN)Deployments:$(NC)"
	@kubectl get deployments -n commerce -l app=order-service

k8s-logs: ## Show application logs
	@kubectl logs -f deployment/order-service -n commerce

k8s-logs-db: ## Show MariaDB logs
	@kubectl logs -f deployment/order-service-mariadb -n commerce

k8s-describe: ## Describe pod (for debugging)
	@kubectl describe pod -n commerce -l app=order-service

k8s-shell: ## Open shell in application pod
	@kubectl exec -it deployment/order-service -n commerce -- /bin/sh
...
k8s-stop: ## Stop application (scale to 0)
	...
	@kubectl scale deployment/order-service -n commerce --replicas=0
	@kubectl scale deployment/order-service-mariadb -n commerce --replicas=0
...
k8s-start: ## Start application (scale to 2)
	...
	@kubectl scale deployment/order-service -n commerce --replicas=2
	@kubectl scale deployment/order-service-mariadb -n commerce --replicas=1
...
k8s-clean: ## Delete all Kubernetes resources
	...
	@kubectl delete deployment,svc,configmap,secret,hpa,pdb -n commerce -l app=order-service 2>/dev/null || true
	@kubectl delete deployment,svc,secret,pvc -n commerce -l app=order-service-mariadb 2>/dev/null || true
...
port-forward: ## Forward application port to localhost:8080
	...
	@kubectl port-forward svc/order-service 8080:80 -n commerce

port-forward-db: ## Forward MariaDB port to localhost:3306
	@echo "$(GREEN)Forwarding MariaDB port 3306...$(NC)"
	@kubectl port-forward svc/order-service-mariadb 3306:3306 -n commerce
...
db-shell: ## Connect to local MariaDB shell
	@docker exec -it order-mariadb mysql -uadmin -padmin1234 commerce-order
...
flyway-history: ## Show Flyway migration history (K8s)
	...
	@kubectl exec deployment/order-service-mariadb -n commerce -- \
		mariadb -uadmin -padmin1234 commerce-order \
...
flyway-history-local: ## Show Flyway migration history (Local)
	@echo "$(GREEN)Flyway Migration History:$(NC)"
	@docker exec order-mariadb mariadb -uadmin -padmin1234 commerce-order \
		-e "SELECT installed_rank, version, description, type, installed_on, execution_time, success FROM flyway_schema_history;" \
