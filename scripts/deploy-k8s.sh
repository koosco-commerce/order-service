...
echo -e "${GREEN}========================================${NC}"
echo -e "${GREEN}Order Service Deployment${NC}"
...
kubectl wait --for=condition=ready pod -l app=order-service-mariadb -n ${NAMESPACE} --timeout=300s
...
# Services
echo "Services:"
kubectl get svc -n ${NAMESPACE} -l app=order-service
...
POD_NAME=$(kubectl get pod -n ${NAMESPACE} -l app=order-service -o jsonpath='{.items[0].metadata.name}')
...
echo ""
echo "Useful commands:"
echo "  - View logs: kubectl logs -f deployment/order-service -n ${NAMESPACE}"
echo "  - Port forward: kubectl port-forward svc/order-service 8080:80 -n ${NAMESPACE}"
echo "  - Get pods: kubectl get pods -n ${NAMESPACE}"
echo "  - Describe pod: kubectl describe pod <pod-name> -n ${NAMESPACE}"
echo ""
echo "Access application:"
echo "  - kubectl port-forward svc/order-service 8080:80 -n ${NAMESPACE}"
