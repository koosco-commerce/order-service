# Kubernetes Deployment Guide for Order Service

## Overview
Complete Kubernetes configuration for the order-service microservice with MariaDB database.
...
kubectl exec deployment/mariadb -n commerce -- \
  mysqldump -uroot -proot commerce-order > backup.sql
```

### Restore Database
```bash
kubectl exec -i deployment/mariadb -n commerce -- \
  mysql -uroot -proot commerce-order < backup.sql
```

### Access Database Shell
```bash
kubectl exec -it deployment/mariadb -n commerce -- \
  mysql -uadmin -padmin1234 commerce-order
```
...
kubectl set image deployment/order-service \
  order-service=order-service:v2.0.0 -n commerce
...
kubectl rollout undo deployment/order-service -n commerce --to-revision=2
```
...
# Test connection from order-service pod
kubectl exec -it deployment/order-service -n commerce -- \
...
```bash
# Recent logs
kubectl logs deployment/order-service -n commerce --tail=100

# Follow logs
kubectl logs -f deployment/order-service -n commerce

# Logs from all replicas
kubectl logs -l app=order-service -n commerce
```
